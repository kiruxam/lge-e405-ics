...
static int write_SilentReset_flag(const char *val, struct kernel_param *kp)
{
	...
	if(silent_reset_buf[0] == '1'){
		...
		lge_hw_reset_enable_disable(1);
	}
	else{
		set_kernel_silencemode(1);
		lge_hw_reset_enable_disable(0);
	}
...
{
	int size = lge_emmc_misc_read(36, silent_reset_buf, SILENT_RESET_SIZE);
	buf[SILENT_RESET_SIZE-1] = '\0';
	printk("%s,lge_silent_reset_flag=%d \n", __func__, get_kernel_silencemode());
	printk("%s,silent_reset_buf=%s \n", __func__, silent_reset_buf);
	return size;
}
