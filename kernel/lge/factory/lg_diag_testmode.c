...
extern void set_operation_mode(int isOnline);	/* LGE_CHANGE : Test Mode(Flight Mode) 2012-07-03 peter.jung@lge.com Modification for minimum current at SMT. */
...
/* LGE_CHANGE_E : Flight Test Mode [End] */
	...
	switch(req_ptr.test_mode_req.sleep_mode){	
		...
		case FLIGHT_MODE_ON:
	  	case FLIGHT_KERNEL_MODE_ON:                        
	  	{
			 ...
			 * 2012-07-03, peter.jung@lge.com
			 * Modification for minimum current at SMT.
			 */
			testMode_sleepMode = TRUE;
			LGF_SendKey(KEY_POWER);
			/* LGE_CHANGE_E : Test Mode(Flight Mode) [End] */
...
void* LGF_TestScriptItemSet(test_mode_req_type * pReq, DIAG_TEST_MODE_F_rsp_type * pRsp)
{
	...
	default:
		pRsp->test_mode_rsp.test_mode_test_scr_mode = pReq->test_mode_test_scr_mode;
		pRsp->ret_stat_code = TEST_NOT_SUPPORTED_S;
		break;

#if 0 // not supported (M4)
