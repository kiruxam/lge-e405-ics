...
/* LGE_CHANGE_S, [20120622][youngbae.choi@lge.com]
*  Let's check at the chargerlogo exec*/
...
#ifdef CONFIG_LGE_SILENCE_RESET
	if(kernel_silence_mode == 1)
		lge_set_reboot_reason(CRASH_REBOOT); // original senario of silent reset		
	else if(kernel_silence_mode == 0)
		lge_set_reboot_reason(CRASH_REBOOT_FAKE); // display kernel crash mode
	else
		lge_set_reboot_reason(CRASH_REBOOT); // in case of exceptional value for lge_silent_reset_flag, do original senario of silent reset
#else
