...
static int msm_batt_get_batt_chg_status(void)
{
	...
	// LGE_CHANGE_E [peter.jung@lge.com], 2012-05-29 [stop]
	} else {
...
/* LGE_CHANGE_S [peter.jung@lge.com][2012-06-08] */
/* power off when battery is just removed */
#include <linux/reboot.h>	/* For kernel_power_off() */

/* LGE_CHANGE_S [peter.jung@lge.com][2012-06-14] */
/* side effect : power off when battery is just removed [*/
#define	LAST_BATT_VALID		(1)
#define   LAST_BATT_INVALID   (0)
unsigned char last_batt_status_was = LAST_BATT_INVALID;
/* ]*/
static void msm_batt_update_psy_status(void)
{
   ...
   */
	...
	printk(KERN_INFO " msm_batt_info : %d    charger : %d  ,\n", msm_batt_info.charger_type , charger_type);
...
/* LGE_CHANGE_S [peter.jung@lge.com][2012-06-08] */ 
/* power off when battery is just removed */
//				pr_info(KERN_INFO "battery removed Power off\n");
//				kernel_power_off();
		// do_exit(0);
		}
...
//	pr_info(KERN_INFO "[peter]last_batt_status_was : %d  - %d\n",last_batt_status_was , msm_batt_info.batt_valid);
	if(last_batt_status_was == LAST_BATT_VALID && msm_batt_info.batt_valid == 0) {
	...
	switch(battery_status) {
			...
			case BATTERY_STATUS_BAD_TEMP:
			case BATTERY_STATUS_BAD:
				last_batt_status_was = LAST_BATT_VALID;
				break;
			default :
				last_batt_status_was = LAST_BATT_INVALID;
				break;
	}
/* ] */
